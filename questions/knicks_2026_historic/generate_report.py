#!/usr/bin/env python3
"""
Generate the PDF report for the 2026 Knicks historic-playoffs analysis.
Run after knicks_2026_historic.py — all PNGs and RESULTS.md must exist.

Narrative prose is read from FINDINGS.md (## sections), in document order, with
no hardcoded section list. Charts referenced as ![caption](file.png) inside a
section are injected inline; RESULTS.md is reproduced verbatim in the appendix.
To change report text, edit FINDINGS.md.
...
def _cover(sections: dict) -> list:
    ...
    return [
        Spacer(1, 0.5 * inch),
        Paragraph("Did the 2026 Knicks Have a Historic Playoff Run?", _STYLES["cover_title"]),
        Paragraph("The 2025–26 New York Knicks in Context", _STYLES["cover_sub"]),
        Spacer(1, 0.15 * inch),
...
def _appendix_results() -> list:
    ...
    flowables: list = [
        ...
        Paragraph(
            ...
            "each time the analysis is run. Sections appear in the same order as "
            "this document.",
            _STYLES["body"],
        ),
        Spacer(1, 0.1 * inch),
...
def _check_prerequisites() -> None:
    ...
    if missing:
        ...
        print("  MPLBACKEND=Agg python3 knicks_2026_historic.py\n")
        print("Then retry:\n")
        print("  python3 generate_report.py")
        sys.exit(1)


def build_report(output_path="knicks_2026_historic_report.pdf"):
    ...
    story += [
        ...
        Paragraph(
            ...
            "against historical playoff runs. See Appendix A for the full analysis "
            "output.",
            _STYLES["note"],
        ),
    ]
