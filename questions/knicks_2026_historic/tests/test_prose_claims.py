...
Guards are recorded by the analysis (``FACTS.guard``) next to the numbers they
rest on and dumped to ``docs/knicks_2026_historic_guards.json`` by the pipeline.
This test reads that file so a claim can't silently go stale: if the underlying
condition flips, the build turns red and names the phrase to rewrite.
"""
import os

from knicks_2026_facts import load_guards

GUARDS_PATH = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "docs", "knicks_2026_historic_guards.json"
)
