...
These drop-offs are sometimes called power laws, which has a precise meaning: value falls by a roughly constant percentage with each step down the ranks. That pattern shows up as a straight line once both axes are stretched onto a log scale. By that test, some systems qualify and some do not. Holding close to a straight line across the top 50: Game Score, PER, Win Shares, DBPM, VORP. Bending instead, with a rate of decline that changes as rank rises: WS/48, BPM, OBPM. So "the lines are power laws" is true for some systems and not others, and the chart below is the way to tell them apart.

![On log-log axes a power law is a straight line; the curved lines are not power laws.](../generated/images/powerlaw_fits.svg){#fig-powerlaw .collapsible}
