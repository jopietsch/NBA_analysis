...
``docs/PROJECT_facts.json`` and ``docs/PROJECT_results.md``
are both produced by ``PROJECT_analysis.py``, but via separate code paths
...
On a brand-new project where facts.json is empty ({}), this test passes trivially
(no facts to check). Populate ALLOW_ABSENT after the first full pipeline run for
any fact that is derived/reformatted and doesn't appear verbatim in results.md.
"""
import json
import os

_DOCS = os.path.join(os.path.dirname(os.path.dirname(__file__)), "docs")
_FACTS = os.path.join(_DOCS, "PROJECT_facts.json")
_RESULTS = os.path.join(_DOCS, "PROJECT_results.md")

# Facts whose value is derived/reformatted and so does not appear verbatim in
# results.md. Keep minimal; each entry is a deliberate "not in results.md by
# design" statement. Populate after the first pipeline run.
