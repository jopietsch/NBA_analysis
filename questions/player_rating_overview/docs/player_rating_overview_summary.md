...
**The impact metric sees something the box scores don't.** RAPM, built only from which lineups outscored their opponents, barely moves together with the box-score systems (about 0.22 on a 0-to-1 scale, where those systems sit far higher with each other). That gap is the point: RAPM is meant to catch the off-ball and defensive value the box score misses. But one season of it is noisy, so the raw 2024-25 list puts a reserve, Isaiah Joe, on top, which is why every published version leans on a box-score starting point.
