...
That is one thread of a larger problem: there is no single best way to rate NBA players, and the systems disagree in ways that matter for players on losing teams, defensive specialists, and high-usage scorers.
...
A reconstruction bug once made that independence look like noise; with it fixed, RAPM+prior, which steadies the noisy lineup data with a box-score estimate, is a usable impact metric, led by the MVP tier (Shai Gilgeous-Alexander, Nikola Jokić, Giannis Antetokounmpo).
It is as steady from year to year as BPM, and across the 29 seasons it can be computed it forecasts next-season team results better than any box score.
