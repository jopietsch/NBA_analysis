...
This report computes its own RAPM with exactly this estimator. Each 2024-25 game's play-by-play is reconstructed into possessions; the design matrix gives every player two columns, one marked +1 on the possessions they played offense and one marked −1 on the possessions they played defense, the target is the offense's points per 100 possessions, and λ is chosen by five-fold cross-validation. The prior is the zero-mean one above, with no SPM term. That bare setup is the reason the single-season estimate is noisier than EPM or LEBRON.

The report also computes a stabilized version, RAPM_MY, that adds the two pieces the published metrics rely on. First, a non-zero prior: instead of shrinking each coefficient toward zero, the ridge shrinks it toward the player's Box Plus/Minus (the offensive column toward OBPM, the defensive toward DBPM). This is the prior-mean ridge, implemented by the substitution γ = β − prior, so the same RidgeCV solver fits the residual and the prior is added back. BPM stands in for the optimized SPM priors that EPM and LEBRON use. Second, multi-year pooling: three seasons of possessions are stacked with linear recency weights (most recent season weighted highest) and fit together, which is how RPM and EPM cut single-season noise. RAPM_MY, combined only, is the version that feeds the consensus. It does not match EPM or LEBRON, whose edge comes from player-tracking inputs this project does not have.
