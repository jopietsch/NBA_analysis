...
The outcome is a useful surprise. PER, the oldest and simplest per-minute box score, rebuilt about 72% of the differences between teams, more than the plus/minus metrics that are explicitly built to track team results. One caveat keeps this from being a verdict on which system is best. The recomputed BPM and VORP here are approximations (see the note on the recomputed formulas below), so their weaker showing partly reflects a noisy recompute rather than a real shortcoming. The fair takeaway is narrow: a plain box-score productivity score already carries most of the signal about which teams win, and this comparison should be rerun against published impact metrics before ranking the systems against each other.
