...
This is the prior-mean ridge, implemented by the substitution γ = β − prior: the solver fits the residual γ and the prior is added back.
The prior version also swaps the cross-validated penalty for a single fixed, strong one (`RAPM_PRIOR_STRENGTH` in `nbakit/ratings.py::rapm`, a plain `Ridge` rather than `RidgeCV`).
Cross-validation minimizes possession-by-possession prediction error and settles on a penalty too weak to pull a moderate-minute player off a lucky lineup context, so the fixed strong penalty is used instead: it collapses low-possession players onto their box-score prior while heavy-minute players, backed by far more possessions, still move to what the lineup data shows.
BPM stands in for the optimized SPM priors that EPM and LEBRON use.
Second, multi-year pooling: several seasons of possessions are stacked with linear recency weights (most recent season weighted highest) and fit together, which is how RPM and EPM cut single-season noise.
...
### Split-half reliability: how we validated RAPM

A from-scratch RAPM can print plausible-looking numbers and still measure almost nothing, so the question is not whether it produces ratings but whether those ratings track a stable, real player quality.
The measure for that is **reliability**: the correlation between two independent measurements of the same thing, running from 0 (the two are unrelated, pure noise) to 1 (they agree exactly).
In classical test theory, the framework that treats an observed rating as a true value plus measurement error, the reliability is also the share of the spread in the ratings that reflects real differences between players rather than noise, which is what puts a rough usability line near 0.5.
Below it noise dominates and the ratings cannot be trusted to rank individual players; 0.5 to 0.7 is where a single-number rating becomes genuinely useful, about where the best public impact metrics sit.

**Split-half reliability** applies that idea to one body of possession data.
Randomly halve the possessions, fit RAPM independently on each half, then correlate the two resulting sets of player ratings.
If RAPM is capturing real signal, a genuinely good player looks good in both halves and the two agree; if it is fitting lineup luck, each half chases its own noise and they diverge.
It is the same logic as handing two graders the same essays and checking whether their scores line up.
