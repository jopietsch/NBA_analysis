...
Scope: `$ARGUMENTS` is the doc path(s) to review. If empty, review the current project's `docs/<project>_findings.md` and `docs/<project>_summary.md`.

This command does NOT restate each review's rules. Each subagent reads the relevant skill file and follows it, so the three reviews can never drift from their standalone versions.
