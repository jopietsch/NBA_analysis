...
- `SUMMARY.md` — standalone one-page summary built around the three core charts (the decline `nba_home_court_advantage_season.png`, the box-score mediation `nba_home_court_mediation.png`, the rebounding collapse `nba_home_court_rebounding.png`) and the three questions; has a generated PDF (`generated/SUMMARY.pdf`). It quotes headline numbers from `home_court_FINDINGS.md`, so **whenever those headline figures change in `home_court_FINDINGS.md`, update `SUMMARY.md` to match**; **whenever `SUMMARY.md` changes, regenerate its PDF** with `python3 generate_doc_pdf.py SUMMARY.md`
...
# Regenerate the one-page summary PDF (run after editing SUMMARY.md)
python3 generate_doc_pdf.py SUMMARY.md
