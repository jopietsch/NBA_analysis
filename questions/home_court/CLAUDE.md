...
├── tests/         — all test files and test data (tests/data/)
...
- `docs/home_court_FINDINGS_OUTLINE.md` — condensed section-by-section outline of `home_court_FINDINGS.md` with every stat and conclusion, cross-referenced to `RESULTS.md`; has a generated PDF (`generated/home_court_FINDINGS_OUTLINE.pdf`). **Whenever `home_court_FINDINGS_OUTLINE.md` changes, regenerate its PDF** with `python3 generate_doc_pdf.py docs/home_court_FINDINGS_OUTLINE.md` (the general Markdown renderer — no dedicated script)
- `docs/home_court_SUMMARY.md` — standalone one-page summary built around the three core charts (the decline `nba_home_court_advantage_season.png`, the box-score mediation `nba_home_court_mediation.png`, the rebounding collapse `nba_home_court_rebounding.png`) and the three questions; has a generated PDF (`generated/home_court_SUMMARY.pdf`). It quotes headline numbers from `home_court_FINDINGS.md`, so **whenever those headline figures change in `home_court_FINDINGS.md`, update `home_court_SUMMARY.md` to match**; **whenever `home_court_SUMMARY.md` changes, regenerate its PDF** with `python3 generate_doc_pdf.py docs/home_court_SUMMARY.md`
...
- `docs/home_court_STATS_EXPLAINER.md` / `STATS_TUTORIAL.md` — hand-edited methods companions to `RESULTS.md`; each has a generated PDF (`generated/home_court_STATS_EXPLAINER.pdf` here; `../generated/STATS_TUTORIAL.pdf` in the parent directory). **Whenever either markdown is edited, regenerate its PDF** with `python3 generate_doc_pdf.py docs/<FILE>.md` (see Commands)
...
# Regenerate a stats doc PDF + HTML (run after editing docs/home_court_STATS_EXPLAINER.md or STATS_TUTORIAL.md)
python3 generate_doc_pdf.py docs/home_court_STATS_EXPLAINER.md
python3 generate_doc_pdf.py ../STATS_TUTORIAL.md

# Regenerate the findings outline PDF + HTML (run after editing docs/home_court_FINDINGS_OUTLINE.md)
python3 generate_doc_pdf.py docs/home_court_FINDINGS_OUTLINE.md

# Regenerate the one-page summary PDF + HTML (run after editing docs/home_court_SUMMARY.md)
...
# Run a single test
python3 -m pytest tests/test_nba_home_court_advantage.py::TestClassName::test_method_name
...
All generated artifacts (every PNG chart and every PDF) are written to the `generated/` directory, which is gitignored. `docs/RESULTS.md` is the exception — it is committed. Markdown image references in `docs/` point at `../generated/<chart>.png`.
...
- when home_court_FINDINGS.md is edited, regenerate the PDF report with `python3 generate_report.py`
- when home_court_FINDINGS.md is edited, also update `docs/home_court_FINDINGS_OUTLINE.md` to match (it's a condensed outline of every stat and conclusion); then regenerate the outline PDF with `python3 generate_doc_pdf.py docs/home_court_FINDINGS_OUTLINE.md`
