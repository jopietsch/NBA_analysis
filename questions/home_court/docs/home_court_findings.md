...
![How many times the decline changed pace, and when. Left: season home win % with a fitted line for each number of breaks: no break (grey dashed), one break (blue solid), two breaks (red dash-dot), three breaks (green dotted); the most likely break years are labeled, and the one-break slopes are shown bottom-left. Right: if the decline has a single break, how likely each year is to be it; 1999 is the most likely, holding about half the probability, with a likely range of 1992–2003.](../generated/images/home_court_bayesian_changepoint.svg){#fig-bayesian-changepoint}
...
![Top/bottom 15 referees ranked by home foul differential (≥50 playoff games). Values are shrunken estimates: raw differences shrink toward the league mean to account for sample size.](../generated/images/home_court_referee_rankings.svg){#fig-referee-rankings}
...
![Franchise home court advantage, regular season and playoffs. Bars and points carry 95% confidence bands: in the regular season most franchises sit clearly apart, but in the playoffs the bands are wide and overlapping, so the franchise differences there are mostly noise.](../generated/images/home_court_team_hca.svg){#fig-team-hca}
