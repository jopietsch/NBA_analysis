...
The breakdown also holds when the straight-line assumption behind it is dropped. A more flexible win model, one that lets the four factors bend and feed off each other instead of adding up in a straight line, splits each game's result among them and then reads off how much each factor's advantage shrank over 40 years. It lands on the same picture: shooting, rebounding, and turnovers each carry a substantial slice of the regular-season decline, fouls the smallest at about 14% (the straight-line table put it a little higher, near 18%), and the slices add back to the same total drop. The playoffs split the same way. Because this method shares none of the simpler one's assumptions, the fact that both reach the same split means it is a feature of the games themselves, not a result of the calculation method.
...
A last worry: maybe the categories just stand in for some hidden cause the data never recorded. A standard check measures how big such a hidden cause would have to be to explain a category's link to winning away. For fouls it would have to be more strongly tied to both fouls and winning than the foul advantage itself is, explaining more than 28.9% of the gap the box-score factors don't already cover; for shooting the bar is higher still, 60.5%. Both are hard to dismiss this way. This shows how much a hidden cause would have to explain before it could undercut the story; it does not prove the categories cause home wins.
