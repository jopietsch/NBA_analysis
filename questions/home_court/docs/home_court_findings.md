...
The decline is a smooth, gradual drift, not a staircase of sudden drops at rule changes. The data strongly support at least one change in the slope: somewhere along the way, the pace of the decline shifted. But they can't say how many times. Two such bends and three fit the 40-year line about equally well, and even a single bend can't be ruled out; the evidence gives no clear reason to prefer one count over another. Every version does place a bend in or near the 1990s, around the hand-checking era. Some add a later bend around 2020, which most likely reflects the COVID disruption and the accelerating recent decline rather than a clean event in that single year. The one thing the data can't do is pin the exact year: the best single estimate sits near 1999, but it carries a margin of roughly a decade, about 1993 to 2002.
...
None of this shows up as a sudden cliff. Every change is a gentle bend in the pace of decline, never a one-season drop. And three-point shooting and home court move in step over time: the rise in threes doesn't run a year ahead of the decline, nor the decline a year ahead of the threes. Neither one leads.

![How many times the decline changed pace, and when. Left: season home win % with a fitted line for each number of breaks: no break (grey dashed), one break (blue solid), two breaks (red dash-dot), three breaks (green dotted); the most likely break years are labeled, and the one-break slopes are shown bottom-left. Right: if the decline has a single break, how likely each year is to be it; 1999 is the most likely, holding about half the probability, with a likely range of 1992–2003.](../generated/nba_home_court_bayesian_changepoint.svg){#fig-bayesian-changepoint}
