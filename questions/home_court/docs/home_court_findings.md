...
The decline has two layers. One is **slow, continuous erosion** of about a quarter of a point per year, the box-score categories in Section 3 grinding down over decades. The other is **two sharp drops** layered on top of it. "Drops" here means two stretches where the slope steepened, not one-season cliffs. The change-point tests in Section 3 can resolve only the first of them: they place a slope change in the late 1990s, with no discrete level jump (which is why the formal stability tests still read the full 40 years as a smooth drift). The second steepening, after 2017, falls at the end of the data where those tests have little power; it shows up instead as a step down in the era-by-era win rates, not as a dated break.
...
The decline is a smooth drift, not a staircase of rule-change shocks. A Bayesian model comparison strongly favors at least one slope change over a single straight-line trend (a Bayes factor of 18 against the no-break model). Past that, the evidence thins. A two-break model is only about twice as likely as a one-break model (P=66% vs. P=32%), too close to call one structure over the other. And the model cannot date the breaks. Even the single best-fitting break carries a bootstrap range of roughly a decade (about 1993 to 2002), so the model's nominal break years (near 1999 for one break; near 1992 and 2003 for two) mark a broad late-1990s-to-early-2000s window, not specific seasons. This doesn't contradict the frequentist QLR result: the QLR finds the one year a single break fits best, while the Bayesian comparison asks how many breaks the series needs and answers "at least one, with a wide location." The CUSUM test still finds no structural instability across the full window, consistent with each break being a gradual slope change rather than a discrete level jump. Three-point attempt rate also does not predict next season's home win rate, and Granger tests confirm neither series leads the other.
