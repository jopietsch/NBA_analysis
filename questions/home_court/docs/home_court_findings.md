...
![Regular season vs. playoff home win % per season, 1983–84 through 2025–26. Dashed lines are overall linear trends. Background shading marks rule-change eras; red dots mark COVID-impacted seasons.](../generated/images/nba_home_court_advantage_season.svg){#fig-advantage-season}
...
![Box-score category shares of the home-court advantage, regular season vs. playoffs.](../generated/images/nba_home_court_mediation_level.svg){#fig-mediation-level width=50%}
...
The Four Factors together account for roughly **96%** of the regular-season decline. Rebounding and turnovers carry the most at roughly **30%** and **27%**, together more than half the drop. Shooting (eFG%), where the three-point shift registers, accounts for about **21%**, and the narrowing whistle for about **18%**. In free throw terms: home teams once attempted roughly 2 more free throws per game than visitors; that edge has narrowed to under half a free throw. In the playoffs the same four categories capture only about 67% of the decline, with only the foul and rebounding trends statistically solid; the playoff breakdown is consistent with the regular-season story, not established on its own. (These shares show where the decline registers in the box score, not a claim about ultimate causes.) The sections below trace how each piece works.

![Box-score category shares of the 40-year home-court decline, regular season vs. playoffs. Rebounding here is total boards (offensive + defensive); the OREB and DREB breakdown is in the rebounding chart further below.](../generated/images/nba_home_court_mediation_decline.svg){#fig-mediation-decline width=50%}

![Home minus away differentials over time: free throw attempts per game (the whistle's practical effect), FG%, eFG%, 3PA rate, 3P%, and FT%. Each panel shows the per-season gap; a trend toward zero means that component of the home-team edge is narrowing. The rebounding and turnover differentials are covered below.](../generated/images/nba_home_court_advantage_differentials.svg){#fig-advantage-differentials}
...
![Distribution of per-official home foul bias by era, playoffs. The spread has compressed: newer officials are not only less biased on average but more uniform in their calling. Chart covers 1995 onward, the earliest era with sufficient per-official playoff data.](../generated/images/nba_home_court_referee_era.svg){#fig-referee-era}
...
![Does each category's decline survive after accounting for three-point volume? Each bar is the share of that category's yearly decline that remains once three-point volume is accounted for: 100% means the decline has nothing to do with three-point shooting, 0% means it is entirely the three-point story, below zero means three-point volume more than fully accounts for it. In the regular season the shooting decline is entirely absorbed by three-point volume, while the rebounding decline is essentially untouched; the playoff bars are mostly small-sample noise (greyed), with only rebounding holding up.](../generated/images/nba_home_court_3pa_control.svg){#fig-3pa-control}

The decline is smooth, not a staircase of sudden drops. Statistical changepoint testing finds evidence for at least one bend in slope around the late 1990s, consistent with the hand-checking era, but can't pin the year within a decade or determine how many bends there are.

![How many times the decline changed pace, and when. Left: season home win % with a fitted line for each number of breaks: no break (grey dashed), one break (blue solid), two breaks (red dash-dot), three breaks (green dotted); the most likely break years are labeled, and the one-break slopes are shown bottom-left. Right: if the decline has a single break, how likely each year is to be it; 1999 is the most likely, holding about half the probability, with a likely range of 1992–2003.](../generated/images/nba_home_court_bayesian_changepoint.svg){#fig-bayesian-changepoint}
...
![Left: league average attendance per game vs. regular-season home win %, 2000–2026, with crowds holding near capacity (and lately setting records) while home court keeps falling. Right: 2020–21 home win % by game attendance, where an empty arena erases the advantage that even a small crowd restores.](../generated/images/nba_home_court_attendance.svg){#fig-attendance}
...
![Top/bottom 15 referees ranked by home foul differential (≥50 playoff games). Values are shrunken estimates: raw differences shrink toward the league mean to account for sample size.](../generated/images/nba_home_court_referee_rankings.svg){#fig-referee-rankings}
...
![Franchise home court advantage, regular season and playoffs. Bars and points carry 95% confidence bands: in the regular season most franchises sit clearly apart, but in the playoffs the bands are wide and overlapping, so the franchise differences there are mostly noise.](../generated/images/nba_home_court_team_hca.svg){#fig-team-hca}
...
![Franchise HCA in the early era (1984–2001, blue) and recent era (2002–26, green), sorted by early-era HCA. Every franchise shifted left. The dashed vertical lines mark the league average in each era.](../generated/images/nba_home_court_team_hca_era.svg){#fig-team-hca-era}
