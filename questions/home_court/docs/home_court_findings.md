...
**It all shows up in the box score.** The four categories together account for about 95% of the home advantage in the regular season, and a similar share in the playoffs. Shooting is the largest piece, more than 40%, followed by rebounding. The weights of each category on the home win rate have held nearly constant across all six eras, so the overall decomposition is not an artifact of mixing different effects from different periods: the game got fairer, not different in how its components combine.
