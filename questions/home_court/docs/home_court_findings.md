...
The Four Factors together account for roughly **96%** of the regular-season decline. In the playoffs the same four categories capture only about 67% of it, with only the foul and rebounding trends standing clearly above the noise, so the playoff breakdown is consistent with the regular-season story rather than established on its own. (These shares show where the decline registers in the box score, not a claim about ultimate causes.) The sections below trace how each piece works.
