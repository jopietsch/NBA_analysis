...
Home court advantage in the NBA has been shrinking for four decades. This report asks four questions about it: **Has it really changed? What creates it? What drove it down? And what didn't?**
...
The breakdown also holds when the straight-line assumption behind it is dropped. A more flexible win model, one that lets the four factors bend and feed off each other instead of adding up in a straight line, splits each game's result among them and then reads off how much each factor's edge shrank over 40 years. It lands on the same picture: shooting, rebounding, and turnovers each carry a substantial slice of the regular-season decline, fouls the smallest at about 14%, and the slices add back to the same drop (about 9.4 pp). The playoffs split the same way. Because this method shares none of the simpler one's assumptions, the agreement means the box-score breakdown is a property of the games, not of how it was measured.
...
A last worry: maybe the categories just stand in for some hidden cause the data never recorded. A standard check measures how big such a hidden cause would have to be to explain a category's link to winning away. For fouls it would have to be more strongly tied to both fouls and winning than the foul advantage itself is, explaining at least 28.9% of what the model leaves unexplained; for shooting the bar is higher still, 60.5%. Both are hard to dismiss this way. This bounds how solid the box-score story is rather than proving the categories cause home wins.
...
- **The turnover gap closed**, accounting for about 27% of the decline. About half traces to the perimeter shift (arc attempts generate fewer live-ball turnovers than drives); the other half is separate from it.
...
**The breakdown was re-run with no straight-line assumption.** The split of the decline across the four categories comes from a model that adds them up in a straight line. A second, more flexible model that lets the categories bend and feed off each other reaches the same split: shooting, rebounding, and turnovers each carry a substantial share and fouls the smallest, adding back to the same drop of about 9.4 pp. Because the two methods share no assumptions, the breakdown is a property of the games, not of the simpler model.

**The drivers were tested against a hidden cause.** Maybe a category is just standing in for something the box score never recorded. There is a standard way to ask how strong such a hidden cause would have to be to explain a link away: it would have to be tied to both the category and winning. For the shooting link, a hidden cause would have to explain more than 60.5% of everything left unexplained in both the shooting gap and who wins; for the foul link, more than 28.9%. Those are demanding thresholds, so the strongest links are hard to wish away. This bounds how solid the story is; it does not prove the categories cause home wins.
