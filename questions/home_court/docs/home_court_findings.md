...
Both major drivers appear to be approaching natural limits. Three-point attempts now make up about 40% of shots, a share that can't grow indefinitely. The league offensive rebounding rate has fallen from 33% to 26% and is approaching a floor below which teams can't realistically retreat further from the glass. If both forces are near their asymptotes, the pace of decline should slow. The empty-arena data provides the best guide to a lower bound: with no crowd, no foul bias, and no preparation edge, home teams still won 51%. Something near that is probably where the long-run trend is headed. The mechanism the data most consistently supports, analytical knowledge diffusing uniformly across all 30 teams, is not specific to basketball. Every major professional sport heavily investing in the same tools is likely on the same trajectory, though that is a prediction the data here can't directly test.
...
The shape of the decline carries its own message. It is a smooth, continuous drift, not a staircase of shocks: the CUSUM test finds no structural instability across 40 seasons. A related result confirms this: three-point attempt rate does not lead home court advantage from one season to the next. Both series co-move, pulled by the same underlying force rather than one causing the other. The data is most consistent with information diffusion as that force. As analytical approaches to shot selection, scouting, and preparation spread uniformly across all 30 teams, strategies converged toward the same optima regardless of venue, and the home team's advantage from familiarity and practice compressed alongside them. The result is what gradual, statistically stable erosion across four decades looks like.
