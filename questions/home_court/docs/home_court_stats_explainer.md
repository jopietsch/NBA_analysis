...
**What the results mean.** The single linear trend is effectively ruled out: the model strongly favors at least one slope change, with BF(k=1 vs k=0)=18.0 (strong on the Jeffreys scale). Beyond that, the evidence is much weaker than the raw probabilities suggest. The two-break model (P=66.3%) edges the one-break model (P=31.9%), but that is a Bayes factor of only about 2.1, which is "not worth more than a bare mention" on the Jeffreys scale. The data supports a break; it does not reliably distinguish one break from two. It also cannot date them: even the single best-fitting break carries a bootstrap range of roughly a decade (the QLR puts it at 1993–2002), so the nominal MAP years (1999 for k=1, with a 95% HPD of 1992–2003; 1992 and 2003 for k=2) should be read as a broad late-1990s-to-early-2000s window, not specific seasons. The k=2 break locations in particular have no reported uncertainty interval and are not separately identified. For k=1, the posterior-weighted slopes are −0.584 pp/yr before the break and −0.255 pp/yr after.

This resolves the QLR–CUSUM tension without contradicting either. The QLR asks "if there is exactly one break, where does it best fit?" and answers 1999. The CUSUM asks "does the trend wander outside what a single straight line can absorb?" and says no, because each break is a slope moderation, not a level jump. The Bayesian model asks "how many breaks, integrating over all locations?" and answers "at least one, location uncertain." It leans toward two breaks over one, but only mildly (a 2:1 Bayes factor), so the safe reading is that the late-1990s slope change is well established while the case for a distinct second break near 2003 is suggestive, not settled.
