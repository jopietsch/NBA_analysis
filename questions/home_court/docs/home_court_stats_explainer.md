...
the statistical approach, why that approach was chosen, what the results mean, and, where a section has one, **why its figure takes the form it does** (a `Why this
...
about −0.25 pp/year (≈ −10 pp over 43 seasons, p < 0.001 in both, OLS R² = 0.74: year alone explains nearly three-quarters of season-to-season variation in HCA).
The playoffs decline at a similar rate (−0.22 pp/yr, total ≈ −9.5 pp) but with
much wider confidence intervals and a far lower R² (0.19), because each playoff
...
cloud (R² = 0.74) against the noisy playoff one (R² = 0.19). The era shading and
...
**What the results mean.** The single linear trend is effectively ruled out: the model strongly favors at least one slope change, with BF(k=1 vs k=0)=14.2 (strong on the Jeffreys scale). Past that, the evidence splits. The posterior allocates 40.5% to k=2 (best-fit breaks: 1992 and 2020), 38.8% to k=3 (best-fit breaks: 1988, 1998, 2020), and 19.3% to k=1 (best-fit break: 1999, 95% HPD 1992–2003). The k=2 and k=3 models are essentially tied: BF(k=3 vs k=2)=1.0, meaning the data offer no reason to prefer three breaks over two. All three multi-break structures place a break in the late 1980s to mid-1990s window, consistent with the hand-checking period. The 2020 break appearing in both k=2 and k=3 likely reflects the COVID disruption and the accelerating modern decline; the underlying slope change began post-2017, but the Bayesian model locates the break closer to 2020 than to 2017. Break years are not precisely datable: the single best-fitting break carries a bootstrap range of roughly a decade. For k=1, the posterior-weighted slopes are −0.577 pp/yr before the break and −0.255 pp/yr after.

This resolves the QLR–CUSUM tension without contradicting either. The QLR asks "if there is exactly one break, where does it best fit?" and answers 1999. The CUSUM asks "does the trend wander outside what a single straight line can absorb?" and says no, because each break is a slope moderation, not a level jump. The Bayesian model asks "how many breaks, integrating over all locations?" and answers "at least one, with the evidence split between two and three, location uncertain."

**Why this chart.** Two panels for two purposes. The left panel overlays the k=0 (grey dashed), k=1 best-fit (blue solid), k=2 best-fit (red dash-dot), and k=3 best-fit (green dotted) fitted lines on the season-level time series: putting all four models together makes the structural comparison immediate and shows where each model's kinks fall. The right panel is a bar chart of the posterior distribution over break year for k=1, with the 95% HPD highlighted: it shows both *where* the model places the break and *how concentrated* the posterior is: the dominant mass at 1999 with the HPD spanning 1992–2003 carries more information than a single best-fit year would.

---

## 4. Foul & Shooting Differentials by Era (`run_differential_analysis`)

**The data.** Per-game home-minus-away differentials in seven box-score metrics (personal fouls, free throw attempts, FG%, eFG%, 3PA rate, 3P%, FT%) for all
...
share of FGA taken from four zones (paint, mid-range, corner 3, above-break 3) from the NBA's shot-location tables, available only from 1996–97 (30 regular
...
**Why an LPM, not a logit.** The whole point is the additive decomposition, contributions that sum exactly to the level and to the trend. That additivity
...
reference. Putting all three on one panel makes the main claim immediately visible (the channel line tracks the actual season-to-season zigzag while the trend line
...
**Why this section exists.** The mediation (Section 7) is an accounting
decomposition: it splits the home advantage into channel shares but assumes the
channels are the path the advantage takes. If some variable the box score never
recorded is associated with both a channel and the outcome, that channel's
coefficient is biased and its share is off. This section bounds that risk with
**sensitivity to unmeasured confounding** in the sense of Cinelli & Hazlett
(2020): two summaries, the **partial R²** and the **robustness value (RV)**, that
say how strong a hidden confounder would have to be to overturn a channel's
estimated effect.

**The approach.** Refit the full mediation model `home_win ~ year +
efg_pct_diff + foul_diff + tov_diff + reb_diff` by OLS, regular season and
playoffs separately, using **classical (non-robust) standard errors and residual
degrees of freedom**, the sensemakr standard. For a channel with t-statistic
`t` on `dof` residual degrees of freedom:
...
Both are reported as percentages. Higher RV means the link is harder to explain
away. The cluster-by-year p-value from Section 7 is printed alongside for
context, but the RV is computed from the classical t-statistic, not the
clustered one.

**What the results mean.** In the regular season the shooting channel carries a
robustness value of 60.5% against a partial R² of
48.1%, and the foul channel a robustness value of
28.9% against a partial R² of
10.5%. A confounder would have to explain a large
share of the leftover variation in both the channel and home wins to erase
either link, which is a demanding requirement. The playoffs give similar values
on far fewer games. This bounds robustness to an omitted cause; it is not a proof
of causation, since the design is observational and the channels remain proximate
box-score expressions of the advantage.

**Why no chart of its own here.** The companion figure
(`home_court_mediation_sensitivity.svg`) is a horizontal bar of each channel's
robustness value, regular season and playoffs; taller bars resist a hidden cause.
...
pace is U-shaped while HCA fell monotonically, so the two can't even share a trend, and the flat scatters show the weak season-level association, before the
...
**The data.** Per-game attendance scraped from Basketball-Reference, which, unlike NBA.com, publishes the gate for every game. It is reliable only from
...
deliberately separates. Panel 1 (dual-axis time series) answers crowd *size*: attendance flat near capacity while HCA falls. Panel 2 is a dose-response bar chart
...
road teams adapt as a series progresses (which would show as a negative slope, home win % falling in later games).
...
seed, is 63.8%. The "road teams figure it out by game 6" story has no support: what looks like series dynamics is just which arena the game is in, plus the
...
boundary (−6.8 pp, p = 0.002). But HCA was *also* declining smoothly throughout, so any late-period dummy would look negative even if the format change did
...
HCA runs about +7.2 pp higher than regular-season HCA for the same franchises: home court is worth more in the postseason on average, though the gap varies
...
falls while the top rises, the distribution is genuinely widening: polarization is real.
