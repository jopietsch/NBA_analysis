...
- **Non-parametric cross-check (SHAP):** flexible model agrees closely with the linear breakdown on fouls (14% vs 18%), turnovers, and rebounding, summing to the same ~9.4 pp drop; shooting is the exception (SHAP 35% vs linear 21%, outside the linear model's own [11, 28]% range) → three of four channels don't hinge on linearity, but shooting's exact share is method-sensitive, not settled. *RESULTS: NON-PARAMETRIC CHANNEL DECOMPOSITION.*
