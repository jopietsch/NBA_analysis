...
The playoffs don't close so neatly. The same four categories capture only about 67% of the postseason decline, leaving roughly a third of it unaccounted for — a gap that doesn't surface in any single box-score column. It most likely reflects something the numbers can't see: a crowd-effect compression that set in after 2018, as playoff road teams became more battle-tested and harder to rattle in hostile buildings. For the regular season the books essentially balance; for the playoffs, an honest share of the decline remains unexplained.
