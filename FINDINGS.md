...
Two things are happening at once, and keeping them apart is the key to the whole report. Underneath everything is a **slow, continuous erosion** — about a quarter of a percentage point a year, every year — that traces back to the on-court channels in Section 3 grinding down gradually. Layered on top are **two sharper drops**, or waves. The first was a steep fall in the mid-1990s, when regular-season home win % dropped from about 65% to 60% in a single stretch — the steepest move in the dataset. The line then held roughly flat for the better part of two decades before a second slide, beginning after 2017, pushed it below 56%.

These two waves are *not* extra causes stacked on top of the gradual decline. Each is a moment when one specific force shoved the same trend harder. The first wave is a one-time rule shock — the 1994–95 hand-checking crackdown (Section 4), the lone rule change in 40 years that left a mark. The second wave is the three-point explosion (Section 3) hitting full stride, as the share of shots taken from deep leapt from about a quarter to nearly half. So the abrupt drops and the gradual drift are the same story at two speeds: the channels wearing down year by year, punctuated twice by a force big enough to register as a step. One note on bookkeeping: the three-point surge behind the second wave shows up *inside* the box-score channels (it lands on the shooting line in Section 3), while the hand-check rule behind the first wave sits *outside* them, as a discrete event — which is why their causes are explained in different sections.
