...
These two drops are *not* extra causes stacked on top of the gradual decline. Each is a moment when one specific force shoved the same trend harder. The first drop is a one-time rule shock — the 1994–95 hand-checking crackdown (Section 4), the lone rule change in 40 years that left a mark. The second drop is the three-point explosion (Section 3) hitting full stride, as the share of shots taken from deep leapt from about a quarter to nearly half. So the sharp drops and the gradual drift are the same story at two speeds: the box-score categories wearing down year by year, punctuated twice by a force big enough to register as a step. One note on bookkeeping: the three-point surge behind the second drop shows up *inside* the box-score categories (it lands on the shooting line in Section 3), while the hand-check rule behind the first drop sits *outside* them, as a discrete event — which is why their causes are explained in different sections.
...
The decline is real and one-directional: a slow, four-decade erosion with two sharper drops layered on it — a one-time jolt from the 1994–95 hand-check rules (a discrete rule shock, outside the box-score categories) and the post-2017 three-point surge (which registers inside them, on the shooting line). It is not the artifact of any single rule change. The data supports three main explanations, plus a quieter fourth:
